...
if (isset($_REQUEST['trainer'])) {
  ...
  $user_check_query = "SELECT * FROM coach WHERE coach_id='$trainer_id' LIMIT 1";
  ...
  if (count($errors) == 0) {
    ...
    $query = "INSERT INTO coach (coach_id,name,time,mobileno,facility_id) 
          VALUES('$trainer_id','$name','$time','$mobileno','$pay_id')";
    $sql=mysqli_query($conn, $query);
    if ($sql) {
    $msg="<div class='alert alert-success'><b>Coach added successfully</b></div>";
    }else{
      $msg="<div class='alert alert-warning'><b>Coach not added</b></div>";
    }
  }
}
...
<div class="container">
    ...
    ?>
		...
		<label class="mt-3">COACH NAME</label>
